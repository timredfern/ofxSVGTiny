								  ...
								  struct svgtiny_parse_state state)
{
	...
	for (unsigned int i = 0; i < map->length(); i++)
	{
		...
		if (strcmp(name, "cx") == 0)
		{
			x = svgtiny_parse_length(content, state.viewport_width, state);
		}
		else if (strcmp(name, "cy") == 0)
		{
			y = svgtiny_parse_length(content, state.viewport_height, state);
		}
								...
								struct svgtiny_parse_state state)
{
	...
	for (unsigned int i = 0; i < map->length(); i++)
	{
		...
		if (strcmp(name, "x1") == 0)
			x1 = svgtiny_parse_length(content, state.viewport_width, state);
		else if (strcmp(name, "y1") == 0)
			y1 = svgtiny_parse_length(content, state.viewport_height, state);
		else if (strcmp(name, "x2") == 0)
			x2 = svgtiny_parse_length(content, state.viewport_width, state);
		else if (strcmp(name, "y2") == 0)
			y2 = svgtiny_parse_length(content, state.viewport_height, state);
	}
	...
	*height = state.viewport_height;
	...
	for (unsigned int i = 0; i < map->length(); i++)
	{
		...
		if (strcmp(name, "x") == 0)
			...
			*width = svgtiny_parse_length(content, state.viewport_width, state);
		else if (strcmp(name, "height") == 0)
			*height = svgtiny_parse_length(content, state.viewport_height, state);
	}
}
									...
									struct svgtiny_parse_state *state)
{
	...
	for (unsigned int i = 0; i < map->length(); i++)
	{
		...
		if (strcmp(name, "fill") == 0)
			...
			state->stroke_width = svgtiny_parse_length(content, state->viewport_width, *state);
		else if (strcmp(name, "style") == 0)
		{
			...
			if ((s = strstr(style, "stroke-width:")))
			{
				...
				while (*s == ' ')
					s++;
				value = strndup(s, strcspn(s, "; "));
				state->stroke_width = svgtiny_parse_length(value, state->viewport_width, *state);
				free(value);
			}
		}
	}
